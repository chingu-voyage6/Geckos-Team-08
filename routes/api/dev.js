...
// @clients   GET api/dev profile/clienthandle/:clienthandle
// @desc      Get developer profile by clienthandle route
// @access    Private
apiRouter.get('/clienthandle/:clienthandle', passport.authenticate('jwt', { session: false }), (req, res, _next) => {
	const errors = {}; // Initialise errors object for use later

	devProfile
		.findOne({ clienthandle: req.params.clienthandle })
		.populate('client', [ 'name', 'avatar' ])
		.then((dev) => {
			if (!dev) {
				errors.nodevprofile = 'This client has no profile';
		...
		.catch((err) => res.status(404).json({ profile: 'This client has no profile and therefore has no handle' }));
});
...
apiRouter.post('/', passport.authenticate('jwt', { session: false }), (req, res, _next) => {
	...
	if (req.body.clientgroup) devFields.clientgroup = req.body.clientgroup;
